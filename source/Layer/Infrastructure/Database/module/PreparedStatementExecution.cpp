...
PreparedStatementExecution::PreparedStatementExecution(
    duckdb::PreparedStatement& DuckDbPreparedStatement,
...
PreparedStatementExecution::WithoutParameters() && {
    return QueryResultDecoder{
        std::move(*this).Execute(duckdb::vector<duckdb::Value>{}),
    };
}

[[nodiscard]] QueryResultDecoder
PreparedStatementExecution::Execute(
    // NOLINTNEXTLINE(cppcoreguidelines-rvalue-reference-param-not-moved)
    duckdb::vector<duckdb::Value>&& DuckDbValueVector
) && {
    std::unique_ptr<duckdb::QueryResult> QueryResult{
        this->m_DuckDbPreparedStatement.Execute(DuckDbValueVector, true)
    };
