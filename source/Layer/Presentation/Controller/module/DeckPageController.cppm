...
public:
    ...
    DeckPageController&
    operator=(const DeckPageController&) = delete;

    explicit DeckPageController(DeckPageController&&) = delete;
    DeckPageController&
    operator=(DeckPageController&&) = delete;

    ~DeckPageController() noexcept override = default;

    [[nodiscard]] Model::DeckForestModel*
    GetDeckForestModel() noexcept;

    [[nodiscard]] std::expected<void, const char*>
    CreateRootDeck(const std::string&, std::uint8_t) noexcept;

    [[nodiscard]] std::expected<void, const char*>
    CreateChildDeck(const std::string&, const std::string&) noexcept;

    [[nodiscard]] std::expected<void, const char*>
    MoveDeck(const std::string&, const std::optional<std::string>&) noexcept;

    [[nodiscard]] std::expected<void, const char*>
    RenameDeck(const std::string&, const std::string&) noexcept;

    [[nodiscard]] std::expected<void, const char*>
    DeleteDeck(const std::string&) noexcept;

private:
    void
    RefreshDeckForestModel(std::int64_t) noexcept;
};
