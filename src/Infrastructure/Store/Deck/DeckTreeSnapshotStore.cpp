...
[[nodiscard]] std::vector<Domain::Deck::DeckTreeSnapshotNodeData> DeckTreeSnapshotStore::ReadDeckTreeSnapshot(const qint64 AsOfMillisecondsSinceEpoch) {
    ...
    std::vector<Domain::Deck::DeckTreeSnapshotNodeData> DeckTreeSnapshotNodeDataVector{};
    for (auto QueryResultIterator{ QueryResult->begin() }; QueryResultIterator not_eq QueryResult->end(); ++QueryResultIterator) {
        ...
        DeckTreeSnapshotNodeDataVector.emplace_back(
            QString{ QueryResultRow.GetValue<std::string>(0).c_str() },
            QueryResultRow.IsNull(1) ? std::nullopt : std::make_optional(QString{ QueryResultRow.GetValue<std::string>(1).c_str() }),
    ...
    return DeckTreeSnapshotNodeDataVector;
}
}
