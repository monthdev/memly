...
[[nodiscard]] std::vector<Domain::Deck::DeckTreeSnapshotNodeData> DeckTreeSnapshotStore::ReadDeckTreeSnapshot(const std::int64_t AsOfMillisecondsSinceEpoch) {
    ...
    for (auto QueryResultIterator{ QueryResult->begin() }; QueryResultIterator not_eq QueryResult->end(); ++QueryResultIterator) {
        ...
        DeckTreeSnapshotNodeDataVector.emplace_back(QueryResultRow.GetValue<std::string>(0),
