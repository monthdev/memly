...
CREATE MACRO IF NOT EXISTS deck_name_length_ok(s) AS
    length(s) > 0 AND length(s) <= 25;

CREATE MACRO IF NOT EXISTS card_text_length_ok(s) AS
    length(s) > 0 AND length(s) <= 100;
