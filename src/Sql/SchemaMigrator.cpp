...
void ApplySchemaMigrations(duckdb::Connection& DatabaseConnection) {
    std::unique_ptr<duckdb::QueryResult> QueryResult{ DatabaseConnection.Query(Sql::M00_MigrationsTableSql()) };
    ...
    std::array<std::reference_wrapper<std::string()>, 1> MigrationSqlFunctionArray{ Sql::M01_InitialSchemaSql };
    ...
    for (std::size_t UnappliedMigrationVersionIndex{ AppliedMigrationVersionVector.size() };
        ...
        QueryResult = DatabaseConnection.Query(Sql::CreateMigrationSql(), static_cast<std::uint32_t>(UnappliedMigrationVersionIndex + 1));
        if (QueryResult->HasError()) {
            Support::ThrowError(QueryResult->GetError());
        }
    }
...
void SeedTableDefaults(duckdb::Connection& DatabaseConnection) {
    std::array<std::reference_wrapper<std::string()>, 3> SeedSqlFunctionArray{ Sql::CreateDefaultFsrs7SchedulerSql,
